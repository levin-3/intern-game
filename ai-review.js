...
字段要求：
1. endingLevel 根据玩家表现判断，只能是 excellent、normal、funny、hidden 四种之一，并尽量与游戏结局对应。
...
function normalizeReview(review, rawText = "") {
  ...
  const normalized = { ...fallback, ...(review || {}) };
  ...
  if (!["excellent", "normal", "funny", "hidden"].includes(normalized.endingLevel)) {
    normalized.endingLevel = "normal";
  }
