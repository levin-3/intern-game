/**
 * Vercel Serverless Function — AI 职场能力报告生成
 *
 * 用法：POST /api/ai-review
 * Body: { endingKey, endingTitle, endingAdvice, scores, choices }
 *
 * 环境变量（通过 Vercel Dashboard 设置）：
 *   DEEPSEEK_API_KEY  必需
 *   DEEPSEEK_BASE_URL 可选，默认 https://api.deepseek.com
 *   DEEPSEEK_MODEL    可选，默认 deepseek-v4-flash
 *   AI_PROVIDER       可选，默认 deepseek（预留 aliyun）
 */
...
function getProvider() {
  ...
  if (!provider) {
    const err = new Error(`未知 AI_PROVIDER："${providerName}"，请填写 deepseek 或 aliyun`);
    err.statusCode = 500;
    throw err;
  }
  ...
  const choiceText = choices.length
    ? choices
        .map((item, index) => {
          ...
          const text = item.text || item.choice || item.choiceText || "未记录具体选择";
          return `${order}. ${text}`;
        })
        .join("\n")
    : "暂无选择记录";
...
JSON 格式必须如下：
{
  ...
  "suggestion": "下一步优化建议",
  "jobFit": ["适合的实习方向1", "适合的实习方向2", "适合的实习方向3"],
  "sevenDayPlan": [
    "Day 1：具体行动",
    "Day 2：具体行动",
    "Day 3：具体行动",
...
字段要求：
1. endingLevel 只能是 excellent、normal、funny、hidden 四种之一。
2. strengths、weaknesses、jobFit、sevenDayPlan、evidence 必须是数组，不允许为空。
3. evidence 必须至少引用 2 条玩家的具体选择，并说明该选择反映了什么能力。
4. 内容必须具体，结合分数和选择来写，不要空泛鸡汤。
`;
}
...
function normalizeReview(review, rawText = "") {
  const fallback = fallbackReview(rawText);
  const result = { ...fallback, ...(review || {}) };

  for (const key of ["strengths", "weaknesses", "jobFit", "sevenDayPlan", "evidence"]) {
    if (!Array.isArray(result[key]) || result[key].length === 0) {
      result[key] = fallback[key];
    }
  }

  if (!["excellent", "normal", "funny", "hidden"].includes(result.endingLevel)) {
    result.endingLevel = "normal";
  }
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!provider.apiKey) {
      const missingName = providerName === "aliyun" ? "DASHSCOPE_API_KEY" : "DEEPSEEK_API_KEY";
      return res.status(500).json({
        error: `后端缺少 ${missingName} 环境变量，请在 Vercel 项目设置中配置。`,
        hint: "本地开发请在 .env 文件中设置，线上部署请在 Vercel Dashboard → Environment Variables 中添加。"
      });
    }
    ...
    const response = await fetch(`${provider.baseURL}/chat/completions`, {
      ...
      body: JSON.stringify({
        ...
        messages: [
          {
            role: "system",
            content: "你是专业的 AI 求职教练、职场能力分析师和校园招聘产品顾问。请只返回合法 JSON。"
          },
          {
            role: "user",
            content: prompt
    ...
    if (!response.ok) {
      ...
      return res.status(502).json({
        error: `AI 供应商返回错误：${response.status}`,
        message: errorText
      });
    }
    ...
    let review;
    try {
      review = normalizeReview(extractJsonFromText(rawText), rawText);
    } catch (parseError) {
    ...
    return res.status(error.statusCode || 500).json({
      error: "AI 调用失败",
      message: error.message || "请检查 API Key、模型名称和网络。"
    });
  }
}
